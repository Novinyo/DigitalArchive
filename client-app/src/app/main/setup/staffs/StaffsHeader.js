...
import { selectFilteredStaffs, selectSearchText, setStaffsSearchText } from './store/staffsSlice';

function StaffsHeader(props) {
  const dispatch = useDispatch();
  const searchText = useSelector(selectSearchText);
  const filteredData = useSelector(selectFilteredStaffs);
      ...
      <div className="flex flex-col sm:flex-row space-y-16 sm:space-y-0 flex-1 items-center mt-16 -mx-8">
        ...
        >
          ...
          <Input
            ...
            onChange={(ev) => dispatch(setStaffsSearchText(ev))}
          />
        </Box>
        <Button
          className="mx-8"
