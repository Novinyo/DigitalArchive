...
import DocumentTypesTableHead from './DocumentTypesTableHead';
import {
  getDocumentTypes,
  selectDocumentTypes,
  selectDocumentTypesSearchText,
} from '../../store/documentTypesSlice';

function DocumentTypesTable(props) {
  const dispatch = useDispatch();
  const documentTypes = useSelector(selectDocumentTypes);
  const searchText = useSelector(selectDocumentTypesSearchText);
  ...
  useEffect(() => {
    dispatch(getDocumentTypes(false)).then(() => setLoading(false));
  }, [dispatch]);

  useEffect(() => {
    if (searchText.length !== 0) {
      setData(
        _.filter(documentTypes, (item) =>
  ...
  function handleClick(item) {
    console.log(item);
    props.navigate(`/setup/documenttypes/${item.id}`);
  }
    ...
    <div className="w-full flex flex-col min-h-full">
      <FuseScrollbars className="grow overflow-x-auto">
        <Table stickyHeader className="min-w-xl" aria-labelledby="tableTitle">
          <DocumentTypesTableHead
            selectedDocumentTypeIds={selected}
            order={order}
...
export default withRouter(DocumentTypesTable);
